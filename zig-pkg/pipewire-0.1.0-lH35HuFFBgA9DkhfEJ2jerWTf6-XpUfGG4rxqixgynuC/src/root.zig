...
/// What every `Stream` call can fail with.
///
/// Exported because `Stream.open`, `writeAll` and the rest return it, and a
/// caller outside this library otherwise has no way to write that set down —
/// which leaves it declaring `!T` and inferring, where it wanted to name the
/// errors it handles.
pub const StreamError = stream_mod.Error;
...
/// What every `Session` call can fail with, for the same reason
/// `StreamError` is here.
pub const SessionError = session_mod.Error;
