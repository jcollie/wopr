...
one; the lower layers are exported for callers who need to reach past them. The
error sets both of them return are `StreamError` and `SessionError`, so that a
caller can name what it handles rather than inferring it.
